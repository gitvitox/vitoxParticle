...
import de.vitox.particle.two.Particle2;
import de.vitox.particle.two.ParticleGenerator2;
...
public class GuiMainMenu extends GuiScreen implements GuiYesNoCallback
{
    ...
    private ParticleGenerator2 particles2;
    ...
    public void initGui()
    {
        ...
        this.particles2 = new ParticleGenerator2(100, this.width, this.height);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        for (Particle p : particles.particles) {
			for (Particle2 p2 : particles2.particles2) {
				int xx = (int) (MathHelper.cos(0.1F * (p.x + p.k)) * 10.0F);
				int xx2 = (int) (MathHelper.cos(0.1F * (p2.x + p2.k)) * 10.0F);
				boolean mouseOver = (mouseX >= p.x + xx - 100) && (mouseY >= p.y - 100) && (mouseX <= p.x)
						&& (mouseY <= p.y);

				if (mouseOver) {

					if (mouseY >= p.y - 100 && mouseX >= p2.x - 100 && mouseY >= p2.y && mouseY <= p2.y + 150 && mouseX <= p2.x ) {
						GL11.glPushMatrix();
						...
						GL11.glVertex3d(p2.x + xx2, p2.y, 0);
						GL11.glVertex3d(p.x + xx, p.y, 0);
		...
		this.particles2.drawParticles2();
